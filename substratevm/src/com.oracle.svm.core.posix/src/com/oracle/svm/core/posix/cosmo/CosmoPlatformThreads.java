...
import com.oracle.svm.core.posix.cosmo.headers.Pthread.pthread_mutex_t;
...
public final class CosmoPlatformThreads extends PlatformThreads {
    ...
    public ThreadLocalKey createUnmanagedThreadLocal() {
        ...
        CosmoUtils.checkStatusIs0(Pthread.pthread_key_create(key, Word.nullPointer()), "pthread_key_create(key, keyDestructor): failed.");
    ...
    public void deleteUnmanagedThreadLocal(ThreadLocalKey key) {
        ...
        CosmoUtils.checkStatusIs0(resultCode, "pthread_key_delete(key): failed.");
    }
    ...
    public void setUnmanagedThreadLocalValue(ThreadLocalKey key, WordBase value) {
        ...
        CosmoUtils.checkStatusIs0(resultCode, "pthread_setspecific(key, value): wrong arguments.");
    }
}

@TargetClass(value = Thread.class, onlyWith = CosmoLibCSupplier.class)
final class Target_java_lang_Thread {
    ...
    /**
     * Every thread started by {@link CosmoPlatformThreads#doStartThread} has an opaque pthread_t.
     */
    @Inject //
    @RecomputeFieldValue(kind = RecomputeFieldValue.Kind.Reset)//
...
final class CosmoParker extends Parker {
    private static final Unsafe U = Unsafe.getUnsafe();
    private static final long EVENT_OFFSET = U.objectFieldOffset(CosmoParker.class, "event");
    ...
    CosmoParker() {
        ...
        CosmoUtils.checkStatusIs0(Pthread.pthread_mutex_init(mutex, mutexAttr), "mutex initialization");
        CosmoUtils.checkStatusIs0(PthreadConditionUtils.initConditionWithRelativeTime(relativeCond), "relative-time condition variable initialization");
        CosmoUtils.checkStatusIs0(PthreadConditionUtils.initConditionWithAbsoluteTime(absoluteCond), "absolute-time condition variable initialization");
    }
    ...
    private void park0(boolean isAbsolute, long time) {
        ...
        CosmoUtils.checkStatusIs0(status, "park: mutex_trylock");
        ...
        try {
            ...
            CosmoUtils.checkStatusIs0(Pthread.pthread_mutex_unlock(mutex), "park: mutex_unlock");
            /*
    ...
    protected void unpark() {
        ...
        try {
            ...
            try {
                ...
                CosmoUtils.checkStatusIs0(Pthread.pthread_mutex_unlock(mutex), "CosmoParker.unpark(): mutex unlock");
            }
            if (s == 0 && p.isNonNull()) {
                /*
                 ...
                 */
                CosmoUtils.checkStatusIs0(Pthread.pthread_cond_signal(p), "CosmoParker.unpark(): condition variable signal");
            }
        } finally {
            StackOverflowCheck.singleton().protectYellowZone();
        }
