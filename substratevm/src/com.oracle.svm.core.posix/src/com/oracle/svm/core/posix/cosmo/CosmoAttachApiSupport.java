...
public class CosmoAttachApiSupport implements AttachApiSupport {
    private final ReentrantLock lock = new ReentrantLock();
    private final AtomicBoolean shutdownRequested = new AtomicBoolean();
    private State state;
    private CosmoAttachListenerThread attachListenerThread;
    ...
    @Fold
    public static CosmoAttachApiSupport singleton() {
        return ImageSingletons.lookup(CosmoAttachApiSupport.class);
    }
    ...
    public void initialize() {
        ...
        try {
            ...
            if (createListener()) {
                attachListenerThread = new CosmoAttachListenerThread(listener);
...
class CosmoAttachListenerThread extends AttachListenerThread {
    private final int listener;

    public CosmoAttachListenerThread(int listener) {
    ...
    protected AttachOperation dequeue() {
        while (true) {
            ...
            com.oracle.svm.core.posix.cosmo.CosmoAttachListenerThread.PosixAttachSocketChannel channel = new com.oracle.svm.core.posix.cosmo.CosmoAttachListenerThread.PosixAttachSocketChannel(socket);
    ...
    private static class PosixAttachSocketChannel extends AttachSocketChannel {
        ...
        public int read(PointerBase buffer, int size) {
            return CosmoUtils.readUninterruptibly(socket, (Pointer) buffer, size, 0);
        }

        @Override
        protected void write(byte[] data) {
            CosmoUtils.writeUninterruptibly(socket, data);
        }
