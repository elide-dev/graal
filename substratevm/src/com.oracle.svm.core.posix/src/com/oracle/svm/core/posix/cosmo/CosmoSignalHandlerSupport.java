...
import com.oracle.svm.core.posix.cosmo.headers.CSunMiscSignal;
import com.oracle.svm.core.posix.cosmo.headers.Errno;
import com.oracle.svm.core.posix.cosmo.headers.Signal;
import com.oracle.svm.core.posix.cosmo.headers.Signal.SignalDispatcher;
import com.oracle.svm.core.posix.cosmo.headers.Signal.SignalEnum;
...
public final class CosmoSignalHandlerSupport implements SignalHandlerSupport {
    ...
    @Fold
    public static CosmoSignalHandlerSupport singleton() {
    ...
    private static long installJavaSignalHandler0(int sig, long nativeH, boolean isSignalHandlingAllowed) {
        ...
        try {
            ...
            if (sig == SignalEnum.SIGSEGV.getCValue() || sig == SignalEnum.SIGBUS.getCValue()) {
                ...
                if (currentDispatcher.equal(CosmoSubstrateSegfaultHandler.SIGNAL_HANDLER.getFunctionPointer())) {
                    return ERROR_HANDLER;
                }
            }
    ...
    public void stopDispatcherThread() {
        ...
        CosmoUtils.checkStatusIs0(code, "CSunMiscSignal.signalSemaphore() failed.");
    ...
    public void onIsolateTeardown() {
        ...
        CosmoUtils.checkStatusIs0(code, "CSunMiscSignal.close() failed.");
    }
     ...
     */
    @Uninterruptible(reason = "Locking without transition requires that the whole critical section is uninterruptible.")
    public static SignalDispatcher installNativeSignalHandler(int signum, SignalDispatcher handler, int flags, boolean isSignalHandlingAllowed) {
        CIntPointer lock = LOCK.get();
        NativeSpinLockUtils.lockNoTransition(lock);
        try {
    ...
    /** See {@link #installNativeSignalHandler(int, SignalDispatcher, int, boolean)}. */
    @Uninterruptible(reason = "Locking without transition requires that the whole critical section is uninterruptible.")
    public static void installNativeSignalHandler(SignalEnum signum, Signal.AdvancedSignalDispatcher handler, int flags, boolean isSignalHandlingAllowed) {
        CIntPointer lock = LOCK.get();
        NativeSpinLockUtils.lockNoTransition(lock);
        try {
    ...
    private static void installNativeSignalHandler0(int signum, Signal.AdvancedSignalDispatcher handler, int flags, boolean isSignalHandlingAllowed) {
        ...
        CosmoUtils.checkStatusIs0(result, "sigaction failed in installSignalHandler().");
    }
    ...
    private static class DispatcherThread extends Thread {
        ...
        public void run() {
            while (!stopped) {
                ...
                CosmoUtils.checkStatusIs0(code, "CSunMiscSignal.awaitSemaphore() failed.");
...
class CosmoSignalHandlerFeature implements InternalFeature {
    ...
    private static void setSignalData() {
        ...
        CosmoSignalHandlerSupport.singleton().setData(map, supportedSignals);
    }
...
final class IgnoreSignalsStartupHook implements RuntimeSupport.Hook {
    ...
    private static void installNoopHandler(SignalEnum signal, boolean isSignalHandlingAllowed) {
        CIntPointer lock = CosmoSignalHandlerSupport.LOCK.get();
        NativeSpinLockUtils.lockNoTransition(lock);
        try {
            int signum = signal.getCValue();
            PointerBase currentDispatcher = CosmoSignalHandlerSupport.getCurrentDispatcher(signum);
            if (currentDispatcher == Signal.SIG_DFL()) {
                /* Replace with no-op signal handler if no custom one has already been installed. */
                SignalDispatcher newDispatcher = CosmoSignalHandlerSupport.getDefaultDispatcher(signum);
                assert newDispatcher == NOOP_SIGNAL_HANDLER.getFunctionPointer();
                SignalDispatcher signalResult = CosmoSignalHandlerSupport.installNativeSignalHandler0(signum, newDispatcher, Signal.SA_RESTART(), isSignalHandlingAllowed);
